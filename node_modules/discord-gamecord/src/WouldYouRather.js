...
module.exports = class WouldYouRather extends events {
  ...
  async startGame() {
    ...
    .setTitle(this.options.embed.title + " `" + (this.acc + 1) + "/10" + "`")
    .setDescription(`1. ${this.data.option1} \n2. ${this.data.option2}`)
    .setAuthor({ name: this.message.author.tag, iconURL: this.message.author.displayAvatarURL({ dynamic: true }) })
    // .addFields({ name: 'Detalles', value: `**Título:** ${this.data.title}\n**Autor:** ${this.data.author}` })
    ...
    const btn3 = new ButtonBuilder().setStyle("DANGER").setLabel(this.options.buttons.option3).setCustomId('wyr_3').setEmoji('✖️');
  ...
  async gameOver(msg, result) {
    ...
    .setAuthor({ name: this.message.author.tag, iconURL: this.message.author.displayAvatarURL({ dynamic: true }) })
    .setImage('https://i.ytimg.com/vi/qNIYDsqPO_g/maxresdefault.jpg')

    if (result === '1') embed.setDescription(`**1. ${this.data.option1} (${prnt1}%)**\n2. ${this.data.option2} (${prnt2})%`);
    if (result === '2') embed.setDescription(`1. ${this.data.option1} (${prnt1}%)\n**2. ${this.data.option2} (${prnt2})%**`);
    if (result === '3') {
    ...
    if (this.acc >= 10) setTimeout(() => msg.edit({ embeds: [embed2], components: [] }), 3000);
    else setTimeout(async() => await this.startGame(), 3500)
