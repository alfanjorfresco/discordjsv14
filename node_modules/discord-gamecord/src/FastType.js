...
const sentences = require('../utils/sentences.json');
...
module.exports = class FastType extends events {
  constructor(options = {}) {
    ...
    if (!options.sentence) options.sentence = sentences.sentences;
    ...
    this.timeTaken = null;
    this.wpm = 0;
    this.arrayMsg = this.options.sentence[Math.floor(Math.random() * this.options.sentence.length)]
  }
  ...
  async startGame() {
    ...
    .addFields({ name: 'Frase', value: this.arrayMsg.split(' ').map(e => '`'+ e.split('').join(' ')+'`').join(' ') })
    ...
    collector.on('collect', (m) => {
      ...
      return this.gameOver(msg, m.content?.toLowerCase().trim() === this.arrayMsg.toLowerCase());
    })
